...
def main(args):
    ...
    # datasets>=4.0 returns a lazy `Column` from ds[col]; ColBERT's
    # Collection.cast only accepts a real list, so materialize them.
    passage_contents = list(passage_ds["passage_content"])
    passage_ids = list(passage_ds["passage_id"])
